...
const getCaseCategories = (item) => {
  const categories = Array.isArray(item.categories) ? item.categories : [item.category].filter(Boolean);
  return categories.map(normalizeCaseCategory).filter(Boolean);
};
const iconPaths = {
  bone: [
